[[_Declarative_styling_specification]]
...
Vectors support the following binary operators by performing component-wise operations: `===`, `!==`, `+`, `-`, `*`, `/`, and `%`. For example `vec4(1.0) === vec4(1.0)` is true since the stem:[x], stem:[y], stem:[z], and stem:[w] components are equal. Operators are essentially overloaded for `vec2`, `vec3`, and `vec4`.
